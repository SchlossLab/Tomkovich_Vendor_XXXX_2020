...
pcoa_data <- read_tsv("data/process/vendors.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.pick.opti_mcc.0.03.subsample.braycurtis.0.03.lt.ave.pcoa.axes") %>%
...
plot_grid(pcoa_plot_combined, pcoa_exp1, pcoa_exp2, labels = c("Combined Experiments", "Experiment 1", "Experiment 2"), ncol =1, label_x = .086, label_y = 1)+
  ggsave("exploratory/notebook/pcoa_by_exp_bray.pdf", width = 5, height = 11)
...
plot_pcoa_timepoint <- function(df, desired_day){
  pcoa_timeppoint <- ggplot(df, aes(x=axis1, y=axis2, color = vendor)) +
    ...
    labs(x="PCoA 1",
         ...
         alpha= "Day") +
    theme_classic()
  save_plot(filename = paste0("results/figures/pcoa_day_bray", desired_day,".png"), pcoa_timeppoint)
}
...
plot_initial_communities <- pcoa_data %>% filter(day == -1) %>% 
  ...
  scale_colour_manual(name=NULL,
    ...
    labs(x="PCoA 1",
         ...
         shape= "Experiment") +
    theme_classic()+
  ggsave("exploratory/notebook/pcoa_initial_all_bray.pdf")
...
plot_grid(Schloss, Young, Charles_River, Envigo, Jackson, Taconic,
  ...
  ggsave("exploratory/notebook/pcoa_by_vendor_bray.pdf", width = 8.5, height = 11)
...
plot_grid(Schloss_initial, Young_initial, Charles_River_initial, Envigo_initial, Jackson_initial, Taconic_initial,
          ncol = 2, label_x = .086, label_y = 1)+
  ggsave("exploratory/notebook/pcoa_by_vendor_initial_bray.pdf", width = 8.5, height = 11)
...
#                           labels=c(2, 4, 6, 8, 10))+
    ...
    labs(title = NULL,
         ...
         alpha= "Day") +
    theme_classic()+
  ggsave("exploratory/notebook/pcoa_by_MiSeq_run_bray.pdf")
