...
#S12: Compare test AUCs for each model to 0.5. Wilcoxan signed rank test. Null hypothesis is the distribution of test AUCs for each model is symmetric about mu (0.5)
all_vs_random <-  read_tsv("data/process/classification_to_random.tsv") 
#S13: Pairwise comparisons of test AUCs from the 3 models
...
table_S1_S15_sheets <- list("S1_Diversity_Kruskal_Wallis" = diversity_kruskal_stats_adjust, "S2_diversity_pairwise" = diversity_pairwise_results,
                            "S3_PERMANOVAdn1_d0_d1" = adonis_dn1_to_0, "S4_PERMANOVA_source_dn1"  = dn1_source, "S5_PERMANOVA_all_timepoints" = adonis_all_samples,
                            "S6_source_OTUs_day_-1" = kw_w_sig_dn1, "S7_source_OTUs_day_0" = kw_w_sig_d0, "S8_source_OTUs_day_1" = kw_w_sig_d1,
                            "S9_CFU_stats" = cfu_kruskal_stats_adjust, "S10_weight_stats" = weight_kruskal_stats_adjust,
                            "S11_clindamycin_OTUs" = o_dn1to0_pairs_stats_adjust,
