source("code/functions.R")

a <- ggdraw() + draw_image("results/figures/pcoa_day0.png")
b <- ggdraw() + draw_image("results/figures/richness_d0.png")
c <- ggdraw() + draw_image("results/figures/shannon_d0.png")
d <- ggdraw() + draw_image("results/figures/Dn1toD1_otus_d0.png")
e <- ggdraw() + draw_image("results/figures/Dn1toD1_families_d0.png")



plot_grid(a, NULL, b, c, d, e, labels = c("A", "", "B", "C", "D", "E"), label_size = 12, ncol=2)+
  ggsave("results/figures/figure_3.pdf", width=6, height=8.5)+
  ggsave("submission/figure_3.pdf", width=6, height=8.5)

