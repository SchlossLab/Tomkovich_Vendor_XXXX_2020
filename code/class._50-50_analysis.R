...
dayn1_file <- list.files(path= 'data/process/classification', pattern='combined_best_hp_results_dayn1_50', full.names = TRUE) 
dayn1 <- map_df(dayn1_file, read_files) %>% 
  mutate(input_data = "dayn1") 
day0_file <- list.files(path= 'data/process/classification', pattern='combined_best_hp_results_day0_50', full.names = TRUE) 
day0 <- map_df(day0_file, read_files) %>% 
  mutate(input_data = "day0") 
day1_file <- list.files(path= 'data/process/classification', pattern='combined_best_hp_results_day1_50', full.names = TRUE) 
...
ggsave("results/figures/class._logistic_regression_50.pdf", plot = performance)
